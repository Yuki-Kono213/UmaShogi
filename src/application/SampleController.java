								...
								for(int i2 = 0; i2 < HorseElements.size(); i2++) {
									if(HorseElements.get(i2).text().split(" ").length > 22 && LocalDate.parse(HorseElements.get(i2).text().split(" ")[0], 
										...
										if(LocalDate.parse(HorseElements.get(i2).text().split(" ")[0],DateTimeFormatter.ofPattern("yyyy/[]M/[]d")).isAfter(LocalDate.parse(labelRaceDate.getText(), DateTimeFormatter.ofPattern("yyyy/[]M/[]d")).minusYears(1)) && HorseElements.get(i2).text().split(" ")[2].equals(labelRaceRange.getText().substring(0,5))
												&&  (pastMaxGoodTime.equals("一年未走") || (!pastMaxGoodTime.equals("一年未走") && CalcTime(pastMaxGoodTime, HorseElements.get(i2).text().split(" ")[13], HorseElements.get(i2).text().split(" ")[4],pastMaxGoodTime)))) {
		 ...
		 table.getSelectionModel().selectedItemProperty().addListener((observable, oldVal, newVal) ->
	        {
				...
				try {
					...
					for(int i =0 ; i < stageel.size(); i++) {
						...
						else if (cnt * 25 < roundel.size() && Util.returnLocalDicExist(roundel.get(cnt * 25 + 1).text(),cnt)){
							if( i + 1 < stageel.size() && stageel.get(i).attr("href").split("/")[2].contains("race") && Util.returnDateCompare(horseDate.replace("/",""), stageel.get(i).attr("href").split("/")[3].substring(0,8))) {
								boolean flag = true;
								while(flag) {
